...
Релиз про управление местом под вкладки воркспейсов в боковом режиме
(`tab_bar_position = "side"`): теперь пользователь сам решает, сколько вкладок
влезает, — двумя независимыми настройками.

### Высота области вкладок — перетаскиваемая ручка

- Между областью вкладок и стеком навигационных кнопок (Терминал/Git/Files…)
  появилась тонкая drag-ручка. Тянете вверх/вниз — высота области вкладок
  фиксируется, а освободившееся место забирает блок кнопок.
- До этого область вкладок жёстко занимала весь остаток высоты сайдбара
  (`flex-1`); изменить её высоту было нельзя. Теперь `ui.tab_strip_height` (px)
  хранит зафиксированную высоту, `0` — прежнее автоматическое поведение.
- Нижний предел ~120 px, чтобы полный стек кнопок не выдавил вкладки; сам стек
  кнопок при нехватке места скроллится, как и раньше. На свёрнутом иконок-рейле
  ручка скрыта.

### Высота каждой вкладки — числовая настройка

- Settings → Interface → «Side tab height (px)» задаёт высоту отдельной вкладки
  (0 = компактный дефолт, меньше значение — больше вкладок влезает). Применяется
  только в боковом режиме; верхняя панель не затронута (там своя «Tab bar
  height»).

### Прочее

- Новые поля `ui.tab_height` и `ui.tab_strip_height` (`u16`, `#[serde(default)]`)
  — старые `config.toml` парсятся без миграции, обе настройки стартуют с `0`
  (вид приложения для существующих пользователей не меняется, пока они сами не
  потянут ручку или не выставят число).

## v0.1.2 — 2026-07-26

Релиз про одно: интерфейс перестал прятать собственное содержимое. Все семь
дефектов ниже воспроизведены в живом приложении замерами, а не на глаз, и
закрыты регрессионными тестами (`e2e/chrome-layout.spec.ts`, `e2e/sidebar.spec.ts`).

### Вкладки проектов больше не вытесняются кнопками навигации

- **Боковой сайдбар (режим «вкладки сбоку»).** Полоса вкладок и стек кнопок
  делят высоту сайдбара, но у полосы `overflow-y-auto`, из-за чего её
  автоматический минимальный размер равен **нулю**, а стек кнопок был
  `shrink-0` без верхней границы. Каждая новая кнопка отбирала высоту у
  вкладок, пока те не схлопывались в 0 px — замер на окне 660 px показал стек
  высотой **779 px**. Теперь стек ограничен 45 % высоты и скроллится сам, так
  что вкладкам всегда остаётся большинство.
- **Верхняя панель (раскладка по умолчанию) — та же болезнь по горизонтали.**
  С включёнными кнопками полоса вкладок сжималась до **98 px** — уже одной
  вкладки. Панель измеряет доступное место и уводит не влезающие кнопки в меню
  «⋯»; вкладкам гарантированы 240 px, то есть минимум одна целая вкладка плюс
  «+».
- **Левый рельс иконок** просто обрезался краем окна: кнопки ниже были
  недостижимы. Теперь рельс считает вместимость и отдаёт остаток в меню «⋯».
- **Подсказки прокрутки.** У скроллящихся стеков сайдбара появились градиенты
  у краёв, за которые уехало содержимое: Chromium/WebView2 рисует overlay-
  скроллбары, гаснущие в покое, поэтому сам скроллбар подсказкой работать не мог.
- **Кнопка «Все вкладки»** была последним элементом *прокручиваемой* полосы, то
  есть уезжала из виду ровно тогда, когда появлялась. Перенесена в постоянно
  видимый кластер.

### Статус-бар, панели и оверлеи

- **Часы в статус-баре были не видны** — их закрывал блок версии `v0.1.1`,
  который рисуется поверх строки сегментов. Ширина блока теперь резервируется
  симметрично с обеих сторон: перекрытия нет, а центрирование сегментов
  сохраняется.
- **Мёртвые переключатели панелей на узком окне.** Ниже адаптивных брейкпоинтов
  (900 px для правой панели, 700 px для левой) панели скрывались принудительно —
  кнопка загоралась «включено», ничего не появлялось, объяснения не было.
  Брейкпоинт-гейты убраны: панели сами ограничивают ширину (`32vw`), так что на
  окне 850 px панель занимает 272 px, а доку остаётся 578 px.
- **Командная палитра вылезала за нижний край** короткого окна вместе с футером
  подсказок (504 px при высоте окна 480). Диалог стал колонкой, ограниченной
  вьюпортом: уступает список, а не футер. То же применено к переключателю
  проектов.
- **Секции настроек ниже «Статус-бара»** (Горячие клавиши, Разработчик, О
  программе) на низком окне обрезались и были достижимы только через поиск —
  список секций получил собственную прокрутку.
- **Заголовок правой панели** уезжал при прокрутке вместе с кнопками
  «Настроить» и «Скрыть» — стал `sticky`.

### Сборка

- **macOS снова собирается.** Всплывающее окно трея создаётся с
  `.transparent(true)`, но в Tauri этот метод на macOS закрыт cfg-гейтом
  `feature = "macos-private-api"` — сборка v0.1.1 под macOS падала с
  `error[E0599]: no method named 'transparent'`, и в релиз попадали только
  артефакты Windows и Linux. Фича включена вместе с парным флагом конфигурации
  `app.macOSPrivateApi`, так что всплывающее окно трея на macOS выглядит так же,
  как на других платформах. Приватный API macOS закрывает дорогу в Mac App
  Store — Luxor распространяется через GitHub Releases, так что это не имеет
  значения.

### Пайплайн

- **macOS-сборка больше не падает на подписи.** В репозитории нет секретов
  Apple, но workflow всё равно передавал `APPLE_CERTIFICATE` — GitHub
  подставляет пустую строку, а для бандлера Tauri переменная при этом
  «задана»: он звал `security import` без сертификата и валил всю macOS-джобу
  (`SecKeychainItemImport: One or more parameters passed to a function were not
  valid`), так что артефактов под macOS не было вовсе. Теперь переменные
  экспортируются только при реально настроенном сертификате, иначе собирается
  неподписанный бандл. Добавите секреты — подпись включится сама.
- **Зелёные гейты качества.** `cargo fmt` (102 расхождения в 20 файлах) и шесть
  ошибок `clippy -D warnings` чинились впервые с момента, когда Format стал
  падать первым шагом и прятал за собой check, clippy и тесты. Плюс 12 тестов
  Discord-настроек падали на Linux и macOS: подмена `localStorage` шла
  присваиванием, а Bun там отдаёт его как readonly-свойство.
- **RUSTSEC:** взяты совместимые обновления `crossbeam-epoch` 0.9.18 → 0.9.20 и
  `quinn-proto` 0.11.14 → 0.11.16. Оставшиеся четыре предупреждения — это
  `quick-xml`, которому нужен мажорный апгрейд до 0.41: его пока не принимает ни
  один зависимый пакет, и он намеренно оставлен видимым в аудите, а не заглушён.

### Внутреннее

- Логика краёв прокрутки вынесена в `useScrollEdges` (`src/lib/scrollEdges.ts`)
  и переиспользуется полосой вкладок и стеком кнопок вместо копии эффекта в
  `TopBar`.
- Дубль CSS-утилиты `.lx-noscrollbar` / `.lx-no-scrollbar` схлопнут в один класс.
